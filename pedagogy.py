...
"""
Pedagogical helpers:
- Suggest concrete, context-aware CQ answers (not generic placeholders)
- Explain *why* acceptance changed (before/after grounded)
"""
...
# --------- Small text parsers for percentages and goal lines ------------------
...
def find_goal_metrics(ir: ArgumentIR) -> Tuple[Optional[float], Optional[float]]:
    """
    Look for a proposition that encodes a goal with percentages, e.g.:
    "Goal: increase net transit revenue by at least 3% while keeping ridership loss at or below 2%."
    Returns (revenue_target_pct, ridership_loss_cap_pct).
    """
    rev = None; loss = None
    for p in ir.propositions:
        t = p.text.lower()
        if "goal" in t and ("revenue" in t or "ridership" in t):
            # get the first two percents in order
            nums = [first_percent(p.text)]
            # crude: find the second percent after the first
            rest = p.text
...
def find_offpeak_effect(ir: ArgumentIR) -> Tuple[Optional[float], Optional[float]]:
    """
    Look for a proposition with off-peak price increase and ridership effect, e.g.:
    "a 5% off-peak increase ... reduce ridership by ~1.5%".
    Returns (price_increase_pct, ridership_drop_pct).
    """
    dp = None; dq = None
    for p in ir.propositions:
        t = p.text.lower()
        if "off-peak" in t or "off peak" in t:
            ...
            if len(nums) >= 2:
                try:
                    dp = float(nums[0]); dq = float(nums[1])
                    break
                except Exception:
                    continue
    return dp, dq
...
# --------- CQ Answer suggestions ---------------------------------------------

def suggest_cq_answer(ir: ArgumentIR, cq_label: str, inference_label: str) -> str:
    """
    Return a concise, concrete suggestion text tailored to the example where possible.
    Falls back to a generic but informative template.
    """
    cq_lower = cq_label.lower()
    # PracticalReasoning patterns
    if "means_lead_to_goal" in cq_lower:
        ...
        if rev is not None and dp is not None and dq is not None:
            # simple revenue approx: ΔR ≈ ΔP + ΔQ (with ΔQ negative)
            delta_r = dp - dq
        ...
        return ("Provide a quantitative link from premises to the goal (e.g., price change vs. ridership impact) "
    ...
    if "side_effects_acceptable" in cq_lower:
        ...
        return ("Mitigation plan: limit to off-peak periods and add discounts for sensitive groups to keep side effects acceptable.")
    if "better_alternative_absent" in cq_lower:
        alt = find_alt_policy_node(ir)
        if alt:
            return ("Show why fare enforcement alone is insufficient (e.g., already near saturation, high marginal cost), "
                    "or argue for a combined plan where enforcement contributes and a modest off-peak increase closes the gap.")
        return ("Argue that no strictly better feasible alternative achieves the goal, or propose a combined plan with quantified benefits.")
    # CauseToEffect patterns
    if "mechanism" in cq_lower:
        return ("Specify operational steps linking enforcement to recovered revenue (audits, citation rate, compliance), with baseline and expected uplift.")
    if "temporal_precedence" in cq_lower:
        return ("Cite before–after data showing revenue rises following enforcement changes (not preceding them).")
    if "robustness" in cq_lower:
        return ("Provide results across multiple routes/periods to show the effect persists beyond a single case.")
    # Deductive defaults
    ...
    if "rule_applicable" in cq_lower:
        return ("State the warrant/rule precisely and show that its conditions are satisfied in this case.")
    if "term_consistency" in cq_lower:
        return ("Define key terms (e.g., 'ridership', 'revenue') consistently throughout the argument.")
    # default fallback
    return ("Provide a targeted warrant/backing that directly answers this question; add data or a calculation, not generalities.")

# --------- Explain acceptance changes -----------------------------------------

def explain_acceptance_delta(before_E: Set[str], after_E: Set[str],
                             attacks: Set[tuple], labels: Dict[str,str], target: str) -> List[str]:
    """
    Human-readable explanation of why target's status changed, focusing on which attackers
    were neutralized by which accepted nodes.
    """
    lines: List[str] = []
    if target in before_E and target in after_E:
        lines.append("Target was already accepted; plan preserved acceptance.")
        return lines
    ...
    for a in sorted(list(atk)):
        ...
        if target in after_E:
            ...
            if a in after_E:
                lines.append(f"  - {a} ('{a_lab}') is itself accepted and continues to block the target.")
            else:
