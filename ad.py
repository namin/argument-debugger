...
from scheme_layer import SchemeAssigner, SchemeFacts, SCHEMES_LP
...
% -------------------------
% Support graph (closure) with scheme guards
% -------------------------
...
supports(X, Y) :- inference(X, Y), not disabled_inference(X, Y).
supports(X, Z) :- supports(X, Y), inference(Y, Z), not disabled_inference(Y, Z).
...
class ArgumentDebugger:
    ...
    def debug_argument(self, argument_text: str, apply_repair: bool = True) -> Dict:
        ...
        assigner = SchemeAssigner("schemes.json", temperature=0.0)
        sfacts = assigner.analyze(argument, argument_text)
