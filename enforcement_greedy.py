...
def strengthen_within(ir: ArgumentIR, tgt: str, g: AFGraph, budget: int) -> Plan:
    ...
    # prioritize obligations on inferences that directly defend the target (attack neg:target)
    neg_t = f"neg:{tgt}"
    defenders = {a for (a, b) in attacks if b == neg_t}

    # Greedy: answer obligations on direct defenders first
    prioritized = []
    for n in unmet_nodes:
        # which inference does n attack?
        target_inf = None
        for (a, b) in attacks:
            if a == n:
                target_inf = b; break
        if not target_inf:
            continue
        if target_inf in defenders:
            prioritized.append((0, n, target_inf))   # higher priority
        else:
            prioritized.append((1, n, target_inf))
    prioritized.sort()

    steps = 0
    for _, n, target_inf in prioritized:
