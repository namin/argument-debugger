...
# -*- coding: utf-8 -*-
"""
LLM → ARG-IR parser (single semantic parsing).

- Uses llm.py from the repo if available; otherwise, falls back to a heuristic parser.
- Produces a single ARG-IR instance (see arg_ir.py) that downstream compilers can consume.
...
def tolerant_json_extract(s: str) -> Optional[dict]:
    ...
    try:
        ...
        pass
    # 2) find largest balanced object
    candidates = []
    opens = []
...
- "inferences": [{
    ...
    fol?: {premises: [..], conclusion: "...", symbols: {...}}
  }]
...
def _heuristic_parse(text: str) -> ArgumentIR:
    ...
    i1 = Inference(id="i1", from_ids=premises, to=tgt, rule="defeasible", type="deductive", scheme="ModusPonens",
